...
# create a script
# permanent, repeatable, annotated, shareable archive of your analysis
...
8 == 8 # are two things equal?

7 == 8

7 != 8 # are two things not equal?
...
1:50
c(1:50) # not a vector, just telling R to call this out
1:10
...
summary(dat)
str(dat) # diff columns, how R is classifying the columns/variables
...
# subset a row, here showing the 2nd column
dat[2,] # brackets: number in front of comma = row
        #           number behind comma = column

# subset a column
dat[,2] #this shows the 2nd col

# add a new variable
dat$plant <- c(1, 1, 2, 2, 3, 3, 4, 4)

# rename a variable
names(dat)[2] <- "aphids" #col 2 is being renamed to aphid
colnames((dat))

# graph the data
plot(dat$aphids, pch = 19)
boxplot(dat$aphids)
boxplot(aphids ~ trmt, data = dat) #count of aphid by trmt
